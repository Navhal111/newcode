...
public class MainVideoViewnoti extends YouTubeBaseActivity implements YouTubePlayer.OnInitializedListener {
    ...
    private YtFragmentedVideo MainDownloadAudio;
        ...
        whatsapp_share_mp3.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(flagDownload == 1){
                    ...
                    DownloadBAr.setVisibility(View.VISIBLE);
                    try{
                        if(MainDownloadAudio == null){
                            ToastMsgFail("Sorry Problem On Server, Try Again After One Minute");
                        }else{

                            Toast.makeText(MainVideoViewnoti.this, MainDownloadAudio.audioFile.getFormat().getExt(), Toast.LENGTH_SHORT).show();
                            Thread thread = new Thread(){
                                @Override
                                public void run() {
                                    try {
                                        ...
                                        downloadFromUrl(MainDownloadAudio.audioFile.getUrl(),"Sad Status",Title+" $ "+VideoId+"mp3."+MainDownloadAudio.audioFile.getFormat().getExt());
                                    } catch (Exception e) {
                                        e.printStackTrace();
                                    }
                                }
    ...
    private boolean downloadFromUrl(String youtubeDlUrl, String downloadTitle, String fileName) {
        ...
        File downloadfolder = new File(Environment.getExternalStorageDirectory(), "SadStatus");
        //        File downloadfolder = new File(Environment.getExternalStorageDirectory() +
        //                File.separator + "SadStatus");
        if (!downloadfolder.exists()){
            downloadfolder.mkdirs();
        }
        request.setDestinationInExternalPublicDir("SadStatus", fileName);
