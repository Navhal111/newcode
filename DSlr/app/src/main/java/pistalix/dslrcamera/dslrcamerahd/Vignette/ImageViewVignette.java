package pistalix.dslrcamera.dslrcamerahd.Vignette;
...
import pistalix.dslrcamera.dslrcamerahd.ImageEditActivity;
...
public class ImageViewVignette extends ImageViewTouch {
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if (!mVignetteRect.isEmpty()) {
            ...
            final float radiusX = (float) (mVignetteRect.width() / 2 * Math.cos(rad));
            final float radiusY = (float) (mVignetteRect.height() / 2 * Math.sin(rad));
    ...
    private void updateGradientShader(float value, final Paint paint) {
        ...
        mGradientShader = new RadialGradient(
    ...
    private void updateBackgroundMask(int value) {
        if (value >= 0) {
            setPaintColor(ImageEditActivity.mcolor);
        } else {
    ...
    static float hypotenuse(RectF rect) {
        return (float) Math.sqrt(Math.pow(rect.right - rect.left, 2) + Math.pow(rect.bottom - rect.top, 2));
    }

    static float hypotenuse(float left, float top, float right, float bottom) {
        return (float) Math.sqrt(Math.pow(right - left, 2) + Math.pow(bottom - top, 2));
    }
