...
public abstract class DemoStickerView extends FrameLayout implements View.OnClickListener {
    ...
    public DemoStickerView(Context context, AttributeSet attrs, OnTouchSticker onTouchSticker) {
        super(context, attrs);
        init(context);
        this.onTouchSticker = onTouchSticker;
    }

    public DemoStickerView(Context context, AttributeSet attrs, int defStyle, OnTouchSticker onTouchSticker) {
        ...
        this.iv_delete.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                if (DemoStickerView.this.getParent() != null) {
                    ViewGroup myCanvas = ((ViewGroup) DemoStickerView.this.getParent());
                    myCanvas.removeView(DemoStickerView.this);
                }
            }
        });
            ...
            if (view.getTag().equals("DraggableViewGroup")) {
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        DemoStickerView.this.setX(DemoStickerView.this.getX() + offsetX);
                        DemoStickerView.this.setY(DemoStickerView.this.getY() + offsetY);
                        move_orgX = event.getRawX();
                        move_orgY = event.getRawY();
                        break;
                ...
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        this_orgX = DemoStickerView.this.getX();
                        this_orgY = DemoStickerView.this.getY();

                        scale_orgX = event.getRawX();
                        scale_orgY = event.getRawY();
                        scale_orgWidth = DemoStickerView.this.getLayoutParams().width;
                        scale_orgHeight = DemoStickerView.this.getLayoutParams().height;

                        rotate_orgX = event.getRawX();
                        rotate_orgY = event.getRawY();

                        centerX = DemoStickerView.this.getX() +
                                ((View) DemoStickerView.this.getParent()).getX() +
                                (float) DemoStickerView.this.getWidth() / 2;
                        ...
                        centerY = DemoStickerView.this.getY() +
                                ((View) DemoStickerView.this.getParent()).getY() +
                                statusBarHeight +
                                (float) DemoStickerView.this.getHeight() / 2;
                    ...
                    case MotionEvent.ACTION_MOVE:
                        ...
                        if (length2 > length1
                            ...
                            DemoStickerView.this.getLayoutParams().width += offset;
                            DemoStickerView.this.getLayoutParams().height += offset;
                            onScaling(true);
                            //DraggableViewGroup.this.setX((float) (getX() - offset / 2));
                            //DraggableViewGroup.this.setY((float) (getY() - offset / 2));
                        } else if (length2 < length1
                                && (angle_diff < 25 || Math.abs(angle_diff - 180) < 25)
                                && DemoStickerView.this.getLayoutParams().width > size / 2
                                && DemoStickerView.this.getLayoutParams().height > size / 2) {
                            //scale down
                            double offsetX = Math.abs(event.getRawX() - scale_orgX);
                            double offsetY = Math.abs(event.getRawY() - scale_orgY);
                            double offset = Math.max(offsetX, offsetY);
